var check = exports.check = function(jsonScheme, jsonData, jsonSchemePath, orginJsonScheme, originJsonData) {
    ...
    for (var attrName in jsonScheme) {
        ...
        var check_hasOwnProperty = false
        if (jsonData.hasOwnProperty) {
            check_hasOwnProperty = jsonData.hasOwnProperty(attrName)
        } else {
            check_hasOwnProperty = Object.prototype.hasOwnProperty.call(jsonData, attrName)
        }

        if (!check_hasOwnProperty) {
            var result = {
                validate: false,
